...
public class LargeScaleInfoA3 extends HttpServlet {
	...
	/*
	 * Examines cookies from the request to either extract or generate a new
	 * sessionID. Additionally attaches a cookie to the response.
	 */
	private Cookie handleCookie(HttpServletRequest request,
		...
		if (request.getCookies() != null) {
			...
			for (Cookie c : request.getCookies()) {
				...
				ConcurrentHashMap<String, String> parsed = parseCookieValue(c
						.getValue());
				// System.out.println(c.getValue());
				if (c.getName().equals(a2CookieName)
						&& sessionTable.containsKey(parsed.get("sessionID"))) {
					a2Cookie = c;
		...
		if (a2Cookie == null) {
			...
			String cookieVal = sessionID + "_" + sessionValues.get("version")
					+ "_" + getIPPLocal(rpcp);

			a2Cookie = new Cookie(a2CookieName, cookieVal);
		}

		// Add cookie to response regardless, as it always contains new
		// expiration and version information
	...
	/*
	 * Examines the request for the 'cmd' value, and performs the pertinent
	 * action After action is completed, server generates a new cookie to return
	 * to client
	 */
	private void handleCommand(HttpServletResponse response,
			HttpServletRequest request, PrintWriter out, Cookie c) {
		ConcurrentHashMap<String, String> parsed = parseCookieValue(c
				.getValue());
		String sessionID = parsed.get("sessionID");
		...
		String choice = "Just created"; // picked primary, backup, or cache?
		sessionTable.get(sessionID).put("choice", choice);

		// ---------(1) send sessionReadClient to IPP_primary and
		// IPP_backup----------------
		// IPPLocal
		...
		// ---- check to see if IPP_primary or IPP_backup to see if they are
		// equal to IPPLocal ---
		if (IPP_1.equals(IPP_local) || IPP_2.equals(IPP_local)) {
			...
			// Be sure to use rpcp.desanitizeDelimText(message) to desanitize
			// the message for deliminators
			String readResponse = rpcp.sessionReadClient(sessionID, oldVersion,
					IP_addr_1, port_1);
			...
			if (readResponse.equals("notFound")) { // IPP_1 failed
				if (parsed.containsKey("IPP_2")) {
					...
					readResponse = rpcp.sessionReadClient(sessionID,
							oldVersion, IP_addr_2, port_2);
					System.out.println("Read Response from ipp2: "
							+ readResponse);
			...
			// -----------(2)if there is a response from either, use
			// found_Version and new data from now onwards---------------
			// newData = message. If found_Version more recent than your
			// version, then use newData as message
		...
		// Destroy relevant session
			...
			rpcp.sessionDeleteClient(sessionID, oldVersion, local_IP,
					local_port);
			...
			System.out
					.println("Message is greater than 512 bytes. Request ignored and no cookie made");
			return;
		} else {
			String cookieVal = sessionID;
			// update version number
			String newVersion = ((Integer) (Integer.parseInt(oldVersion) + 1))
					.toString();
			...
			/*
			 * TODO: use mbrset, or this: ArrayList<Hashtable<String,String>>
			 * random_mbrset = rpcp.getMembersClient(20, IP_addr_1, port_1);
			 */
			...
			while (write_result == null) {
				...
				// If the mbrSet size is empty, use default IPP backup (i.e.
				// none)
				if (mbrSet.size() == 0) {
					break;
				}

				ConcurrentHashMap<String, String> random_AS = mbrSet.get(rand
						.nextInt(mbrSet.size())); // get random AS
				Calendar discard_time_cal = Calendar.getInstance();
				discard_time_cal.add(Calendar.SECOND, SESSION_TIMEOUT_SECS + 2
						* delta + tau);
				String discard_time = df.format(discard_time_cal.getTime());

				write_result = rpcp.sessionWriteClient(sessionID, newVersion,
						message, discard_time, random_AS.get("ip"),
						random_AS.get("port"));
			...
			// -------(4) make a new cookie with IPP primary and
			// backup----------
	...
	/*
	 * Creates the html with the session's relevant message or a default
	...
	/*
	 * Creates the html displaying the hashed expiration timestamp for the
	 * session
	 */
	private String getSessionExp(String sessionID) {
		...
		if ((sessionID != null) && sessionTable.containsKey(sessionID)) {
			...
			String expirationDate = sessionTable.get(sessionID).get(
	...
	/*
	 * cookieVal is the string used as the value of a CookieIncludes, in this
	 * exact order: sessionID, version, location (arbitrarily long)Each
	 * information of the cookie is in the following format: 'key=value,'
	...
	private ConcurrentHashMap<String, String> parseCookieValue(String cookieVal) {
		...
		if (underscoreParsed.length != 5) {
			System.out.println("array is " + underscoreParsed.length
					+ " components long | " + cookieVal);
		}
		parsed.put("sessionID", underscoreParsed[0] + "_" + underscoreParsed[1]
				+ "_" + underscoreParsed[2]);
	...
	private String getIPPLocal(RPCProtocol rpcp) {
		String ip = "";
		try {
			ip = InetAddress.getLocalHost().getHostAddress() + "_"
					+ rpcp.getUDPLocalPort();
		} catch (UnknownHostException e) {
			e.printStackTrace();
		}
	 ...
	 * compares current time to expiration time of session to see if session is
	 * expired.
	 */
	private class GarbageCollector extends Thread {
		...
		/*
		 * TODO: are we taking care of what they marked us off for last time?
		 * "The clean up process may break the consistency of the hash table..."
		 * 
		 */
		public void run() {
			while (true) {
				for (String sessionID : sessionTable.keySet()) {
					ConcurrentHashMap<String, String> session = sessionTable
							.get(sessionID);
